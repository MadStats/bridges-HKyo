# Variables kept: structure #(8),"LAT_016","LONG_017","STATE_CODE_001"
# year built(27),type of ser(42),total project cost (96)
# average daily traffic(29), year of average daily traffic (30),
# future average daily traffic(114), as well as year of future average daily traffic(115)
...
# downloading straight from url's.  
...
dest="https://www.fhwa.dot.gov/bridge/nbi/2015/delimited/WI15.txt"
tmp = fread(dest) 
tmp = as.tbl(tmp)
#tmp1 = read_csv(dest)
...
x15 = ldply(dest, fread, colClasses = classes)  


# let's dig into the values and see if there are any crazy things going on...
M = x15
M=M[,c("STRUCTURE_NUMBER_008","STATE_CODE_001","LAT_016","LONG_017","YEAR_BUILT_027","SERVICE_ON_042A",
       "STRUCTURE_TYPE_043B","TOTAL_IMP_COST_096","ADT_029","YEAR_ADT_030",
...
###############################
'''
fun = function(x){ return(which(x>2000)) }
...
M = M[,-j]
jold = j
# error for wired char,go through this many times until no 
# error shows up
